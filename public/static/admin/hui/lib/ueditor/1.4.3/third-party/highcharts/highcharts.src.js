 ...
 * Previously this function redirected to jQuery.extra(true), but this had two limitations.
...
function merge() {
	...
	// For each argument, extra the return
	for (i = 0; i < len; i++) {
 ...
 * @param {String} method The name of the method to extra
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			// extra the animate function to allow SVG animations
			var Fx = $.fx,
				Step = Fx.step,
				dSetter,
				Tween = $.Tween,
			...
			// extra some methods to check for elem.attr, which means it is a Highcharts SVG object
		...
		fireEvent: function (el, type, eventArguments, defaultFunction) {
			...
			// raises the warning when they are copied over in the extra statement below.
			//
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				try { // Fails in Firefox if the container has display: none.
					...
					bBox = element.getBBox ?
						// SVG: use extra because IE9 is not allowed to change width and height in case
						// of rotation (below)
...
Tick.prototype = {
	...
	/**
	 * Find how far the labels extra to the right and left of the tick's x position. Used for anti-collision
...
Axis.prototype = {
	...
	/**
	 * These options extra the defaultOptions for left axes
	 */
	...
	/**
	 * These options extra the defaultOptions for right axes
	 */
	...
	/**
	 * These options extra the defaultOptions for bottom axes
	 */
	...
	/**
	 * Set the tick positions to round values and optionally extra the extremes
	 * to the nearest tick
	 */
...
Chart.prototype = {
	...
	getContainer: function () {
		...
		if (useCanVG) {
			// If we need canvg library, extra and configure the renderer
...
Series.prototype = {
	...
	drawTracker: function () {
		...
		if (trackerPathLength && !trackByArea) {
			...
			while (i--) {
				if (trackerPath[i] === M) { // extra left side
					trackerPath.splice(i + 1, 0, trackerPath[i + 1] - snap, trackerPath[i + 2], L);
				}
				if ((i && trackerPath[i] === M) || i === trackerPathLength) { // extra right side
