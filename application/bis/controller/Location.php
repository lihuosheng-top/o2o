...
class  Location extends  Base
{
       ...
       if(request()->isPost())
       {
           ...
           $locationData =[
             'name'=>$data['name'],
               ...
               'ypoint'=>empty($locationResult['result']['location']['lat'])?
                   '':$locationResult['result']['location']['lat'],
               'bis_id'=>$bis_id,
               'open_time'=>$data['open_time'],
               'content'=>$data['content'],
               'is_main'=>1,
               'api_address'=>$data['address'],
               'ciity_id'=>$data['city_id'],
    ...
    {
        $id =input('id',0,'intval');
        //根据id获取bisLocation的信息
        $res =$this->obj->get($id);
        $cities = model('City')->getNormalCitiesByParentId();
        $categories =mmodel('category')->getAllFirstNomalCategories();
